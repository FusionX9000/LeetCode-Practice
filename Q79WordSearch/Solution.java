package LeetCode.Q79WordSearch;
...
public class Solution {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        for (int row = 0; row < R; row++) {
            for (int col = 0; col < C; col++) {
                if (exist(board, row, col, wordArray, 0)) return true;
            }
        }
        return false;
    }

    private boolean exist(char[][] board, int row, int col, char[] wordArray, int pos) {
        System.out.println(row + ", " + col);
        System.out.println(pos);
        if (pos == wordArray.length) return true;

        int R = board.length;
        int C = board[0].length;

        if (!inBound(R, row) || !inBound(C, col) || board[row][col] != wordArray[pos]) return false;

        char ch = board[row][col];
        board[row][col] = '.';
        boolean found = exist(board, row - 1, col, wordArray, pos + 1) ||
        exist(board, row + 1, col, wordArray, pos + 1) ||
        exist(board, row, col - 1, wordArray, pos + 1) ||
        exist(board, row, col + 1, wordArray, pos + 1);
        board[row][col] = ch;
        return found;
    }
    ...
    private boolean inBound(int boundary, int x) {
